...
def validate_intake(document: dict[str, Any]) -> list[Finding]:
    ...
    if document.get("metadata", {}).get("schema_version") not in SUPPORTED_SCHEMAS:
        findings.append(
            finding("INTAKE.SCHEMA_UNSUPPORTED", "unsupported intake schema_version")
        )
    ...
    if not (
        ...
        findings.append(
            finding("INTAKE.SOURCE_MISSING", "source IPLAN id/version/checksum missing")
        )
    ...
    if not (
        ...
        findings.append(
            finding("INTAKE.SCOPE_MISSING", "isolation_scope is incomplete")
        )
    ...
    if fields_missing:
        findings.append(
            finding("INTAKE.TASK_FIELDS_MISSING", "a task lacks id/title/acceptance")
        )
    if dep_unresolved:
        findings.append(
            finding("INTAKE.DEP_UNRESOLVED", "a depends_on references an unknown task")
        )
