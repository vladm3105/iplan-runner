...
def test_pause_then_resume_completes() -> None:
    ...
    resumed = engine.resume(
        MANIFEST, paused.ledger, engine.mock_executor(OUTCOMES), clock=_clock(), ids=IdSource(), sleep=_noop_sleep
    )
...
def test_override_then_resume_retries_blocked_task() -> None:
    ...
    blocked = engine.run(MANIFEST, engine.mock_executor(outcomes), clock=_clock(), ids=IdSource(), sleep=_noop_sleep)
    assert _task(blocked.ledger, "T1")["status"] == "blocked"
    blocker_id = blocked.ledger["blockers"][0]["blocker_id"]

    engine.resolve_blocker(blocked.ledger, blocker_id, "override", {"id": "op", "role": "operator"}, clock=_clock())
    assert _task(blocked.ledger, "T1")["status"] == "pending"

    good = {"T1": {"outcome": "success", "evidence": EV}, "T2": {"outcome": "success", "evidence": EV}}
    resumed = engine.resume(
        MANIFEST, blocked.ledger, engine.mock_executor(good), clock=_clock(), ids=IdSource(), sleep=_noop_sleep
    )
...
def test_resolve_requires_operator() -> None:
    engine = HermesEngine()
    ledger = {
        "blockers": [{"blocker_id": "B1", "task_id": "T1"}],
        "task_ledger": [],
        "isolation_scope": {},
        "execution_log": [],
    }
