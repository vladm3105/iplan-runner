"""Saga retry/compensation/idempotency + lease lifecycle (Hermes)."""
...
def test_retry_then_succeed() -> None:
    ...
    result = engine.run(MANIFEST, engine.mock_executor(outcomes),
                        clock=_clock(), ids=IdSource(), sleep=_noop_sleep, max_retries=2)
...
def test_retry_exhausted_blocks() -> None:
    ...
    outcomes = {"T1": {"attempts": [
        ...
        {"outcome": "failure", "retriable": True}]}}
    result = engine.run(MANIFEST, engine.mock_executor(outcomes),
                        clock=_clock(), ids=IdSource(), sleep=_noop_sleep, max_retries=1)
...
def test_compensation_undoes_partial_write(tmp_path: Path) -> None:
    ...
    result = engine.run(MANIFEST, engine.scripted_executor(spec, tmp_path),
                        clock=_clock(), ids=IdSource(), sleep=_noop_sleep)
    assert _t1(result)["status"] == "blocked"
