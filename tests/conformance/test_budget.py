...
def _cases() -> list[Path]:
    root = _spec.FRAMEWORK / "conformance" / "budget"
    return sorted(p for p in root.iterdir() if p.is_dir())


def _check_fns() -> dict[str, object]:
    fns: dict[str, object] = {}
    for entry in _spec.registry().get("engines", []):
        try:
            module = importlib.import_module(f"{entry['package']}.budget")
        except Exception:
            continue
        fns[entry["id"]] = module.check
    return fns
...
class BudgetTest(unittest.TestCase):
    ...
    def test_budget_parity(self) -> None:
        for case in _cases():
            inp = yaml.safe_load((case / "input.yaml").read_text())
            expect = yaml.safe_load((case / "expect.yaml").read_text())
            decisions = []
            for engine_id, check in self.fns.items():
                decision = check(inp["budget"], inp["usage"])
